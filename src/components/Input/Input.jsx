...
export default function Input(props) {
    ...
    return status
        ? <div {...props}>
            <label htmlFor={labelName} className={`block mb-2 text-sm font-medium ${status === 'success' ? 'text-green-600' : 'text-red-700'}`}>{labelName}</label>
        ...
        : <div>
            <label htmlFor={labelName} className="block mb-2 text-sm font-medium text-black-900 ">{labelName}</label>
